...
class LstmCell(nn.Module):
    def __init__(self,config):
        ...
        self.config=config
        self.W_f=nn.Linear(config.hidden_size+config.input_size,config.hidden_size)
        self.W_i=nn.Linear(config.hidden_size+config.input_size,config.hidden_size)
        self.W_c=nn.Linear(config.hidden_size+config.input_size,config.hidden_size)
        self.W_o=nn.Linear(config.hidden_size+config.input_size,config.hidden_size)
        self.tanh=nn.Tanh()
        self.sigmoid=nn.Sigmoid()

    def forward(self,input,h,c):
        combined = torch.cat((input, h), 1) # [batch_size,1,hidden_size+1]
